...
-- GROUP BY 절 : 같은 값들이 여러개 기록된 컬럼을 가지고 같은 값들을
--							하나의 그룹으로 묶음
...
-- EMPLOYEE 테이블에서 직급 코드가 같은 사람의 직급코드, 급여 평균, 인원수를
-- 직급코드 오름차순으로 조회
...
-- 인원수 오름차순으로 조회
SELECT DECODE( SUBSTR(EMP_NO , 8 ,1) , '1', '남', '2', '여' ) 성별, COUNT(*) 인원수, SUM(SALARY) 급여합
FROM EMPLOYEE
GROUP BY DECODE( SUBSTR(EMP_NO , 8 ,1) , '1', '남', '2', '여' ) 
-- SELECT절 보다 해석순위가 높기때문에 별칭 사용 불가능
ORDER BY 인원수 ASC;
-- SELECT절 보다 해석순위가 낮기때문에 별칭 사용 가능
...
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE	-- DEPT_CODE로 그룹을 나누고, 나눠진 그룹 내에서
...
GROUP BY DEPT_CODE
HAVING AVG( SALARY ) >= 3000000 --> DEPT_CODE 그룹중 급여 평균이 3백만 이상인 그룹만 조회
ORDER BY DEPT_CODE;
...
FROM EMPLOYEE
GROUP BY JOB_CODE
HAVING COUNT( JOB_CODE ) <= 5		-- HAVING절에서는 그룹함수가 반드시 작성된다
ORDER BY JOB_CODE;
