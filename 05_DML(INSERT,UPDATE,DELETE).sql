...
-- DROP TABLE DEPARTMENT2;		// TABLE 삭제
...
--> 선택안된 컬럼은 값이 NULL 이 들어간다(DEFAULT 존재시 DEFAULT 값으로 삽입)

INSERT INTO EMPLOYEE2(EMP_ID, EMP_NAME, EMP_NO, EMAIL, PHONE, DEPT_CODE, JOB_CODE, SAL_LEVEL, SALARY)
							VALUES ('900', '장채현', '901230-123456', 'jang_ch@kh.or.kr', '01012341234', 'D1', 'J7'
											,'S3', 4300000);
...
-- (참고) INSERT 시 VALUES 대신 SUBQUERY 사용 가능
...
SELECT * FROM DEPARTMENT2;	-- 모든 DEPT_TITLE이 '기술연구팀'으로 변경됨
														-- WHERE 조건절이 있어야 하는 이유
...
UPDATE DEPARTMENT2 
SET DEPT_ID = 'D0', DEPT_TITLE = '전략기획팀' -- WHERE절 직전 컬럼 작성시 콤마X
WHERE DEPT_ID ='D9' AND DEPT_TITLE = '총무부';

SELECT * FROM DEPARTMENT2; -- D0	전략기획팀	L1


----------------------------------------------------------------------------------------------------------

-- << UPDATE시에도 SUBQUERY 를 사용 가능 >> --
...
INSERT INTO EMP_M02 VALUES (999, '곽준빈','901220-123456', 'kwack_jb@kh.co.kr', '01032149874', 
...
-- 만약에 WHERE 조건을 설정하지 않을 경우 모든 행이 삭제됨
...
SELECT * FROM EMPLOYEE2 WHERE EMP_NAME = '장채현'; --> 조회결과 없음

ROLLBACK; --> 마지막 COMMIT 시점까지 돌아감

SELECT * FROM EMPLOYEE2 WHERE EMP_NAME = '장채현'; --> '장채현' 정보 복구


-- EMPLOYEE2 테이블 전체 삭제
DELETE FROM EMPLOYEE2;	--> WHERE 절 미작성시 전체 삭제
												--> 24행 삭제
...
-- 롤백 확인
SELECT * FROM EMPLOYEE2; -- 24행 복구


---------------------------------------------------------------------------------------------------------------


-- 5. TRUNCATE (DDL이다, DML 아님)
-- 테이블의 전체 행을 삭제하는 DDL 이다
-- DELETE 보다 수행속도가 빠르다
-- ROLLBACK을 통해 데이터 복구 X

-- TRUNCATRE 테스트용 테이블 생성
...
-- 삭제 확인
SELECT * FROM EMPLOYEE3; 	-- 삭제 O

ROLLBACK;

-- 롤백 후 복구 확인
SELECT * FROM EMPLOYEE3;  -- 복구 O
