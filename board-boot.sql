...
SELECT FILE_NO, FILE_PATH, FILE_ORIGINAL_NAME, FILE_RENAME, MEMBER_NICKNAME, 
			TO_CHAR(FILE_UPLOAD_DATE, 'YYYY-MM-DD') AS FILE_UPLOAD_DATE
FROM "UPLOAD_FILE"
...
-- 특정 게시판에 삭제하지 않은 게시글 목록 조회
-- 단, 최신글이 제일 위에 존재
-- 몇 초/분/시간 전 또는 YYYY-MM-DD 형식으로 작성일 조회

-- ex) 3초전, 15분 전, 10시간 전, 2024-04-22
-- + 댓글 개수
-- + 좋아요 개수
...
-- 현재 시간 - 한 시간 전
SELECT (SYSDATE - TO_DATE('2024-04-22 10:45:30', 'YYYY-MM-DD HH24:MI:SS'))
				* 60 * 60 * 24
FROM DUAL;
...
UPDATE "BOARD_IMG" SET BOARD_NO = '1999';
...
VALUES(6, 1999); -- 1번 회원이 1998번 글에 좋아요를 클릭함

COMMIT;

SELECT * FROM "BOARD_LIKE";

-- 좋아요 여부 확인(1 : 체크 / 0 : 체크X)
...
UPDATE "MEMBER" SET MEMBER_DEL_FL = 'N' WHERE MEMBER_NO = '2';
...
-- 댓글 0개인 BOARD_NO = 2007

INSERT INTO "COMMENT"
VALUES ( SEQ_COMMENT_NO.NEXTVAL, '부모 댓글1', DEFAULT, DEFAULT, 2007, 1, NULL );	-- 2001
INSERT INTO "COMMENT"
VALUES ( SEQ_COMMENT_NO.NEXTVAL, '부모 댓글2', DEFAULT, DEFAULT, 2007, 1, NULL ); -- 2002
INSERT INTO "COMMENT"
VALUES ( SEQ_COMMENT_NO.NEXTVAL, '부모 댓글3', DEFAULT, DEFAULT, 2007, 1, NULL ); -- 2003
...
-- 부모 댓글 1의 자식 댓글
INSERT INTO "COMMENT"
VALUES ( SEQ_COMMENT_NO.NEXTVAL, '부모 1의 자식1', DEFAULT, DEFAULT, 2007, 1, 2001 );

INSERT INTO "COMMENT"
VALUES ( SEQ_COMMENT_NO.NEXTVAL, '부모 1의 자식2', DEFAULT, DEFAULT, 2007, 1, 2001 );


-- 부모 댓글 2의 자식 댓글
INSERT INTO "COMMENT"
VALUES ( SEQ_COMMENT_NO.NEXTVAL, '부모 2의 자식1', DEFAULT, DEFAULT, 2007, 1, 2002 );	-- 2006


-- 부모 댓글 2의 자식 1의 자식 댓글
INSERT INTO "COMMENT"
VALUES ( SEQ_COMMENT_NO.NEXTVAL, '부모 2의 자식1의 자식', DEFAULT, DEFAULT, 2007, 1, 2006 );
...
/* 계층형 쿼리 */
-- PARENT_COMMENT_NO 값이 NULL 인 행을 부모 (LV1)
START WITH PARENT_COMMENT_NO IS NULL
...
(SELECT COMMENT_NO, COMMENT_CONTENT,
		...
		FROM "COMMENT"
		JOIN MEMBER USING(MEMBER_NO)
		WHERE BOARD_NO = 2007) C
WHERE COMMENT_DEL_FL = 'N'	-- 현재 댓글이 삭제되지 않았을 때

OR 0 != (SELECT COUNT(*) FROM "COMMENT" SUB
				WHERE SUB.PARENT_COMMENT_NO = C.COMMENT_NO
				AND COMMENT_DEL_FL = 'N')	-- 자식 없는 댓글은 조회 X
