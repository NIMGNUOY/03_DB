 ...
 *	<< INDEX(색인) >> 
 * 
 * - SQL 명령문 중 SELECT 의 처리 속도를 향상시키기 위해
 * 	컬럼에 대해서 생성하는 객체
 * 
 ...
 * - 조회시 전체 테이블 내용을 조회하는 것이 인덱스가 지정된 컬럼만을 이용해서
 * 		조회 하기 때문에 시스템부하가 낮아져 전체적인 성능 향상이 된다.
 * 
 * -- <인덱스 단점> --
...
SELECT * FROM EMPLOYEE WHERE EMP_ID = 215;	-- 인덱스 사용 O (PK)

SELECT * FROM EMPLOYEE WHERE EMP_NAME = '대북혼';		-- 인덱스 사용 X (제약조건 X)
...
SELECT * FROM TB_IDX_TEST
WHERE TEST_NO = 500000;		-- 인덱스 O (제약조건 PK)
-- 0.002s 
